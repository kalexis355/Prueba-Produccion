...
export class ArchivosPageComponent implements OnInit, OnDestroy {
  ...
  carpetaTieneDelegadoLogueado(carpeta:CarpetaContenido){
  ...
  mostrarMenuContextual(event: MouseEvent, carpeta: any): void {
    ...
    // this.carpetaTieneDelegadoLogueado(carpeta)
    const menuWidth = 230;
    ...
    // Calcular espacio disponible a la derecha para el submenú
    const spaceForSubmenu = windowWidth - (event.clientX + menuWidth);
    this.submenuPosition = spaceForSubmenu < submenuWidth ? 'left' : 'right';

    // Ajusta la posición horizontal si el menú se sale por la derecha
    ...
    // Verifica si hay espacio suficiente abajo
    const spaceBelow = windowHeight - (event.clientY - scrollY);
    if (spaceBelow < menuHeight) {
        // Si no hay espacio suficiente abajo, coloca el menú arriba del cursor
        posY = event.clientY + scrollY - menuHeight;
    }

    // Asegurarse de que el menú esté siempre dentro de la ventana visible
    ...
    const codCortar = localStorage.getItem('CodCarpetACortar');
    const codCopiar = localStorage.getItem('CodCarpetACopiar')
    // this.habilitarOpcionCortar = (serieOrigen && codCortar) !== null;
    // this.habilitarOpcionCopiar= codCopiar !== null
